...
Bird.obj = Bird.def(
  function() {
    this.vtr = new Bird.Vtr(), this.accel, this.width = 600, this.height = 600, this.depth = 300, this.ept, this.area = 200,
    ...
    line: function(b) {
      var _b, totvel = new Bird.Vtr(), cnt = 0;
    ...
    togeth: function(b) {
      var _b, dist, plus = new Bird.Vtr(), dir = new Bird.Vtr(), cnt = 0;
      for (var i = 0, il = b.length; i < il; i++) {
        if (Math.random() > 0.6) continue;
        _b = b[i];
      ...
      if (cnt > 0) { plus.lowscale(cnt); }
      dir.subv(plus, this.pos);
      var l = dir.len();
      if (l > this.mfrc) { dir.lowscale(l / this.mfrc); }
      return dir;
    },
    apart: function(b) {
      var _b, dist, plus = new Bird.Vtr(), rep = new Bird.Vtr();
      for (var i = 0, il = b.length; i < il; i++) {
        if (Math.random() > 0.6) continue;
        _b = b[i];
        dist = _b.pos.dst(this.pos);
...
Bird.Tri = Bird.def(
  function(p1, p2, p3) {
    ...
    rotX: function(a) { this.Vtxs.forEach(function(e) { Bird.Matrix.rotX(e, a); }); },
    rotY: function(a) { this.Vtxs.forEach(function(e) { Bird.Matrix.rotY(e, a); }); },
    rotZ: function(a) { this.Vtxs.forEach(function(e) { Bird.Matrix.rotZ(e, a); }); },
    trans: function(s) { this.Vtxs.forEach(function(e) { Bird.Matrix.trans(e, [s.x, s.y, s.z]); }); },
    vtx: function(idx) {
      for (var i = 0; i < 3; i++) {
...
Bird.Vtr = Bird.def(
  function(x, y, z) {
    ...
    copy: function() { return new Bird.Vtr(this.x, this.y, this.z); },
...
Bird.Matrix = {
  rotX: function(pt, angX) {
    ...
    var asin = Math.sin(angX), acos = Math.cos(angX);
    var xrot = [
      [1, 0, 0],
      [0, acos, asin],
      [0, -asin, acos]
  ...
  rotY: function(pt, angY) {
    var pos = [pt.x, pt.y, pt.z];
    var asin = Math.sin(angY), acos = Math.cos(angY);
    var yrot = [
      [acos, 0, asin],
      [0, 1, 0],
      [-asin, 0, acos]
  ...
  rotZ: function(pt, angZ) {
    var pos = [pt.x, pt.y, pt.z];
    var asin = Math.sin(angZ), acos = Math.cos(angZ);
    var yrot = [
      [acos, asin, 0],
      [-asin, acos, 0],
      [0, 0, 1]
  ...
  mm: function(m1, m2) {
    return [
      m1[0] * m2[0][0] + m1[1] * m2[1][0] + m1[2] * m2[2][0],
...
(function() {
  ...
  function resizeCanvas() {
    ...
    c.height = document.getElementById('contact-bg-wrapper').offsetHeight;
  }
  resizeCanvas();
